...
const AddUser = ({ mode = "add" }) => {
  ...
  const isEditMode = useMemo(() => mode === "edit" && !!id, [mode, id]);
  ...
  const fetchUser = async () => {
    try {
      ...
      if (res?.photo || res?.profileImage || res?.profile) {
        setPreview(res.photo || res.profileImage || res.profile);
      }
      ...
      alert(err?.response?.data?.msg || "Failed to fetch user data");
    } finally {
      setLoading(false);
    }
  };
  ...
  const validateForm = () => {
    ...
    if (mode === "add" && !user.password.trim()) {
      alert("Password is required");
      return false;
    }

    if (
      (mode === "add" || user.password.trim()) &&
